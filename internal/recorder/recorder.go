...
func openInputStream(frame []float32) (*portaudio.Stream, error) {
	...
	return openDeviceStream(dev, frame)
}

// openDeviceStream opens a mono 16 kHz capture stream on a specific device.
func openDeviceStream(dev *portaudio.DeviceInfo, frame []float32) (*portaudio.Stream, error) {
