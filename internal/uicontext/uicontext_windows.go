...
// Capture reads the focused control's text around the caret via UI
// Automation (the Windows analogue of AT-SPI on Linux). On any failure —
// no focus, a control without a TextPattern (terminals, custom editors) —
// HasContext stays false and Apply passes the dictation through unchanged.
